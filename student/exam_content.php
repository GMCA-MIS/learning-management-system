...
$sql = "SELECT * FROM exam WHERE exam_id = $get_id";
    ...
    <div id="content">
        ...
        <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4" style="margin-top: 27px; margin-left: 10px;">
                <h1 class="h3 mb-0 text-gray-800">Exam: <?php echo $examtitle ?></h1>
            </div>
        </nav>
        <!-- End of Topbar -->
                ...
                <div class="col-lg-8">
                    ...
                    <div class="card">
                        <div class="card-body text-center">
                            <h2 class="card-title"><?php echo $examtitle ?></h2>
                        </div>
                    </div>

                    <?php
// Assuming you have the student's ID and exam's ID
// Query to fetch the "taken" status from the student_class_exam table
$sql = "SELECT * FROM student_class_exam WHERE student_id = $student_id AND exam_id = $get_id";
$result = mysqli_query($conn, $sql);
...
if ($result) {
    if (mysqli_num_rows($result) > 0) {
        ...
        // No records found, which means the exam hasn't been taken yet
...
// Determine whether to show the "Take exam" button or the grade
$showButton = ($taken === 'yes') ? false : true;
?>

<div class="card mt-3 moving-background h-100">
    <div class="card-body text-center clr">
        <p class="card-text mx-auto clr"><?php echo $exam_description ?></p>
        <?php if ($showButton) { ?>
            <!-- If the exam is not taken, show the "Take exam" button -->
            <div><a href="take_exam.php?exam_id=<?php echo $get_id ?>" class="exam-button">Take Exam</a></div>
        <?php } else { ?>
            <!-- If the exam is already taken, show the grade from student_class_exam -->
            <div class = "mt-4">
                ...
                <div class = "mt-4"style="font-size: 20px;">
                    ...
                    <div class = "card-body">
                        <div><a href="view_exam_result.php?exam_id=<?php echo $get_id ?>&id=<?php echo $student_id?>" class="exam-button">View Details</a></div>
                    </div>
            </div>
        <?php } ?>
    </div>
