                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
                 ...
                 <!-- Page Heading -->
                    ...
                    <h3 class="h4 mb-0 text-gray-800">
                        <span style="font-weight: lighter;"><?php echo $class_row['class_name']; ?> <?php echo $class_row['subject_title']; ?></span> > Class Members
                    </h3>
...
$check_access_query = mysqli_query($conn, "SELECT * FROM teacher_class_student WHERE student_id = '$student_id' AND teacher_class_id = '$get_id'");
if (mysqli_num_rows($check_access_query) > 0) {
    // The teacher has access to this class

    $my_classmates = mysqli_query($conn, "SELECT * FROM teacher_class_student
        INNER JOIN student ON student.student_id = teacher_class_student.student_id
        WHERE teacher_class_id = '$get_id'") or die(mysqli_error());

    $count_my_classmates = mysqli_num_rows($my_classmates);
    ?>
    <div class="header mt-4 pl-4" style ="backround: white;">
        <h5 class="display-6">Classmate List</h5>
    </div>
    <hr>
    <section class="table__header">
        <?php if ($count_my_classmates > 0) { ?>
            <h6>Number of Students: <span class="badge -info"><?php echo $count_my_classmates; ?></span></h6>
            <div class="input-group">
                <input type="search" placeholder="Search Student...">
            </div>
        <?php } else { ?>
            <center> <div class = "alert alert-warning"> No students enrolled in this subject </div> </center>
        <?php } ?>
    </section>

    <?php
    if ($count_my_classmates > 0) {
    ?>
    ...
    <section class="table__body">
        <table>
            ...
            <tbody>
                <?php
                while ($row = mysqli_fetch_array($my_classmates)) {
