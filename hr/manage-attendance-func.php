...
<?php 
    ...
    {       
        ...
        $exp_timein = $_POST['exp_timein'];
        $exp_timeout = $_POST['exp_timeout'];
        $act_timein = $_POST['act_timein'];
        $act_timeout = $_POST['act_timeout'];

        // Corrected the SQL query, added proper quotation marks
        $query = "UPDATE time_attendance SET 
            `status`='$workstatus',
                expected_timein='$exp_timein',expected_timeout='$exp_timeout',
                     actual_timein='$act_timein',actual_timeout='$act_timeout' WHERE ta_id = '$id'";
        $query_run = mysqli_query($conn, $query);
        ...
        if($query_run) {
            ...
            echo '<script>Swal.fire({
                ...
                window.location.href = "manage-attendance.php?teacherid='.$teacherid.'&startdate='.$startdate.'&enddate='.$enddate.'";
            });</script>';
        } else {
            echo "<script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script>";
            echo '<script>Swal.fire({
