...
if(isset($_POST['delete_coordinators']))
{
    ...
    if($query_run) {
        ...
        echo '<script>Swal.fire({
            ...
            window.location.href = "manage-head-teacher.php?id='.$idget.'";
        });</script>';
    } else {
        echo "<script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script>";
        echo '<script>Swal.fire({
    ...
    {       
        ...
        if($query_run){
            echo '<script>Swal.fire({
                ...
                window.location.href = "manage-head-teacher.php?id='.$id.'";
            });</script>';
        }
    } 
?>
...
if (isset($_POST['add_coordinator'])) {
    ...
    $sql = "SELECT * FROM users WHERE username = '$username' ";
      ...
      if($count >= 1) {
            ...
            VALUES ('$firstname', '$lastname', '../uploads/gmc-logo.png', '$password', 'headteacher','$username')";
            $query_run = mysqli_query($conn, $query);
            $last_id = $conn->insert_id;
            
            $query = "UPDATE department SET user_id = $last_id WHERE department_id = $departmentid";
            $query_run = mysqli_query($conn, $query);
