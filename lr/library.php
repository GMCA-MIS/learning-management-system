...
if (mysqli_num_rows($categoriesResult) > 0) {
    while ($category = mysqli_fetch_assoc($categoriesResult)) {
        ...
        $booksQuery = "SELECT * FROM booklist WHERE category_id = $category_id AND book_status = 'Available' ORDER BY book_id Desc";
        ...
        if (mysqli_num_rows($booksResult) > 0) {
            ...
            while ($book = mysqli_fetch_assoc($booksResult)) {
                ...
                echo '<center> <div class="card-body">';
                echo '<h6 class="card-title" style="color: black;">' . $book_title . '</h6>';
                echo '<h6 class="card-text">' . $publication_year . '</h6>';
                echo '</div> </center>';
                echo '</div>';
                echo '</a>';
            }
