...
$query = "SELECT * FROM coordinators WHERE coordinator_id = $coordinator_id";
...
if ($result) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "No records found for teacher ID: $coordinator_id";
    }
} else {
        ...
        <div class="modal-content">
            ...
            <form method="post" action="" enctype="multipart/form-data">
                <div class="modal-body">
                    <center>
                    <div class="form-group">
                        <input name="image" class="form-control" type="file" id="fileInput" required accept="image/*">
                    </div>
...
<?php
                            if (isset($_POST['change'])) {
								...
								mysqli_query($conn,"update  coordinators set location = '$location' where coordinator_id  = '$coordinator_id' ")or die(mysqli_error());
								
								?>
 
								<script>
								window.location = "lr_profile.php";  
								</script>
...
if (isset($_POST['update_password'])) {
    ...
    // Assuming you already have a valid $coordinator_id

    // Check if the current password is correct (you'll need to fetch the current hashed password from the database)
    $query = "SELECT password FROM coordinators WHERE coordinator_id = ?";
        ...
        if (strlen($newPassword) >= 8 && preg_match('/[A-Z]/', $newPassword)) {
            ...
            if ($newPassword === $repeatPassword) {
                ...
                if ($newPassword !== $currentPassword) {
                    ...
                    $updateQuery = "UPDATE coordinators SET password = ? WHERE coordinator_id = ?";
                    $stmt = $conn->prepare($updateQuery);
                    $stmt->bind_param("si", $hashedNewPassword, $coordinator_id);
                    ...
                    if ($stmt->execute()) {
                        ...
                        echo "<script>
                            Swal.fire({
                                ...
                                window.location = 'lr_profile.php'; // Redirect to profile.php
                            });
                        </script>";
                    } else {
