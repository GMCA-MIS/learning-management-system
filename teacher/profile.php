...
<div id="content-wrapper" class="d-flex flex-column">
    ...
    <div id="content">
        ...
        <?php
// SQL query to fetch data from both teacher and department tables
...
if ($result) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "No records found for teacher ID: $teacher_id";
    }
} else {
...
<?php
                            if (isset($_POST['change'])) {
								...
								mysqli_query($conn,"update  teacher set location = '$location' where teacher_id  = '$teacher_id' ")or die(mysqli_error());
...
if (isset($_POST['update_bio'])) {
    // Check if the 'update_bio' button was clicked

    // Retrieve the new bio from the form
    $newBio = $_POST['desc'];

    // Ensure you have a valid database connection
    // Include your database connection script here, e.g., include('dbcon.php');

    // Assuming you already have a valid $teacher_id

    // Create an SQL query to update the bio
    $updateQuery = "UPDATE teacher SET bio = ? WHERE teacher_id = ?";

    // Prepare the update query
    $stmt = $conn->prepare($updateQuery);

    // Bind parameters
    $stmt->bind_param("si", $newBio, $teacher_id);
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>
            Swal.fire({
                ...
                text: 'There was an error while updating the bio. Please try again.',
        ...
        if (strlen($newPassword) >= 8 && preg_match('/[A-Z]/', $newPassword)) {
            ...
            if ($newPassword === $repeatPassword) {
                ...
                if ($newPassword !== $currentPassword) {
                    ...
                    $updateQuery = "UPDATE teacher SET password = ? WHERE teacher_id = ?";
                    $stmt = $conn->prepare($updateQuery);
                    $stmt->bind_param("si", $hashedNewPassword, $teacher_id);
