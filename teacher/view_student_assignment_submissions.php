...
$query_assignment = mysqli_query($conn, "SELECT fname FROM assignment WHERE assignment_id = '$post_id'");
$row_assignment = mysqli_fetch_assoc($query_assignment);
$assignment_title = $row_assignment['fname'];
...
$gradingQuery = mysqli_query($conn, "SELECT grade FROM student_assignment WHERE student_id = '$studentId' AND assignment_id = '$assignmentId'");
...
if (isset($_GET['student_id'])) {
    ...
    $query = mysqli_query($conn, "SELECT sa.*, a.deadline FROM student_assignment sa
    JOIN assignment a ON sa.assignment_id = a.assignment_id
    WHERE sa.student_id = '$studentId' AND sa.assignment_id = '$post_id'");
...
if (isset($_POST['grade_assignment'])) {
 ...
 if ($grade > $maxScore) {
    ...
    echo "<script>
          Swal.fire({
            title: 'Error',
            text: 'Grade cannot be higher than the maximum score.',
            icon: 'error',
    ...
    $updateQuery = "UPDATE student_assignment SET grade_status = '1', grade = '$grade', comments = '$comments' WHERE student_id = '$studentId' AND assignment_id = '$post_id'";
    
    // Execute the query
    if (mysqli_query($conn, $updateQuery)) {
        // Display a success message using SweetAlert
        $notificationMessage = "<b>$fname</b> has been Graded";
        $link = "view_class_assignment.php?id=".$studentId."&post_id=".$post_id."&get_id=".$get_id;
        ...
        echo "<script>
          Swal.fire({
            title: 'Error',
            text: 'Error updating grade and comments: " . mysqli_error($conn) . "',
            icon: 'error',
            confirmButtonText: 'OK'
          }).then(function() {
            window.location.href = 'view_student_assignment_submissions.php?student_id=$studentId&post_id=$post_id&get_id=$get_id';
...
echo "<script>
          Swal.fire({
            title: 'Error',
            text: 'Error fetching assignment details: " . mysqli_error($conn) . "',
            icon: 'error',
            confirmButtonText: 'OK'
          }).then(function() {
            window.location.href = 'view_student_assignment_submissions.php?student_id=$studentId&post_id=$post_id&get_id=$get_id';
